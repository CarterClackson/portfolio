	...
	return (
		<div className='project'>
			<div>
				...
				{techUsed &&
					techUsed.length > 0 && ( //If techUsed is not null or empty array
						<div>
							...
							<ul className='tech-used'>
								{techUsed.map(
									(
										tech,
										index //map through array and compare to techIcons map for output of icons.
									) => (
										<li key={index}>{techIcons[tech] ? <span>{techIcons[tech]}</span> : tech}</li>
									)
								)}
							</ul>
						</div>
